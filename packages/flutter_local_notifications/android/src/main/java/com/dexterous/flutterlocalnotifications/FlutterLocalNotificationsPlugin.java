...
public class FlutterLocalNotificationsPlugin
  ...
  private static void setMediaStyle(
      NotificationDetails notificationDetails, NotificationCompat.Builder builder) {
    androidx.media.app.NotificationCompat.MediaStyle mediaStyle =
        new androidx.media.app.NotificationCompat.MediaStyle();
    if (notificationDetails.actions != null && !notificationDetails.actions.isEmpty()) {
      int actionsToShow = Math.min(2, notificationDetails.actions.size());
      int[] compactViewIndexes = new int[actionsToShow];
      for (int i = 0; i < actionsToShow; i++) {
        compactViewIndexes[i] = i;
      }
      mediaStyle.setShowActionsInCompactView(compactViewIndexes);
    }
